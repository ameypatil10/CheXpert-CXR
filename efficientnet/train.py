...
#         discriminator.apply(weights_init_normal)
    ...
    scheduler_D = LambdaLR(optimizer_D, lambda epoch: ((epoch < 5) * (epoch+1) + (epoch >= 5) *  np.exp(-2.3*(epoch-5)/5)))

#     scheduler_D = ReduceLROnPlateau(optimizer_D, mode='min', factor=0.1, patience=1, vxerbose=True, cooldown=0)
